...
- **`tt task diff [--task <task-id>] [--repo PATH]`** — Show the diff of all unmerged commits on a task branch since it diverged from its parent branch. Resolves the same range as `tt task revset` (without `--task`: the current task, with an upper bound of `@` if the working copy is non-empty or `@-` if empty; with `--task`: the task bookmark itself), then relays the output of `jj diff --from <fork-point> --to <upper-bound>` directly, where `<fork-point>` is the fork point between the parent bookmark and the upper bound. Exits with an error if the task cannot be located, the bookmark does not exist, or the fork point cannot be resolved. No hooks.
