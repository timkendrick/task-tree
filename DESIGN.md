...
- **`tt worktree show --task <task-id> [--repo PATH]`** — Output the *dedicated* worktree path for the given task or project ID to stdout. Accepts a full task or project ID via the required `--task` flag. The workspace repository root is never a valid result: exits with an error if the task has no dedicated worktree, including when the task is checked out in the main workspace. Exits with a distinct error if the task ID is not found in the repository. Intended for use in shell command substitution.
