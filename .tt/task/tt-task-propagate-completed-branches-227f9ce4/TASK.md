...
Perhaps a better approach would be to modify the `checkin` command such that when a completed branch is checked in, the bookmark should point directly to the 'merge' commit in the parent task's ancestry, *not* a new empty commit that branches off this.

Confirm the current scenario by writing the following `checkin` tests:

- checking in an incomplete branch should leave the task bookmark pointing to an empty commit whose parent is the the merge commit (this test might already exist and should already pass)
- checking in a complete branch (or passing the `--complete` flag when checking in an incomplete branch) should leave the task bookmark pointing to the merge commit itself (this test should fail initially)
- passing the `--propagate` flag when checking in a complete branch (or passing the `--complete` flag when checking in an incomplete branch) should ensure the task bookmark still points to the merge commit (this test should fail initially)
